...
#multithreading=used to perform multiple tasks concurrently(multitasking) Good for I/O bound tasks like reading files or fetching data from APIs threading.Thread(target=my_function)
...
def walk_dog(first):
    time.sleep(8)
    print(f"You finished the walking {first}")
...
chore1=threading.Thread(target=walk_dog,args=("hheehehe",))
chore1.start()
...
pokemon_info=get_pokemon_info(pokeman_name)
...
import sys   #sys full name is system
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Lekhee sudip GUI")
        self.setGeometry(700,300,500,500)
        self.setWindowIcon(QIcon("dv photo.jpg"))
